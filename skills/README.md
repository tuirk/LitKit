...
review operator for LitKit. The agent reads `SKILL.md` and follows
...
Without the skill, agents tend to either (a) explain the kit instead
...
**Claude Code:** Copy or move `litkit/` to:
```
.claude/skills/litkit/SKILL.md
```
Either at the user level (`~/.claude/skills/`) or in the project root.

**Codex:** Copy or move `litkit/` to:
```
.codex/skills/litkit/SKILL.md
```
...
After installing, start a fresh agent session in the LitKit repo root
...
If LitKit changes (new pipeline stage, new export artifact, etc.),
